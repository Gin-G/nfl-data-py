...
def create_player_pool(season, week, main_dataset_path="data/nfl_dataset.csv", 
    ...
    main_dataset_path (str): Path to nfl_dataset.csv
    ...
    print(f"\nAvailable columns in dataset: {df_main.columns.tolist()[:20]}...")
    ...
    for _, player in player_info.iterrows():
        ...
        name_fields = ['player_display_name', 'player_name']
        ...
        # Get sportradar_id (CORRECTED: was sportradar_player_id)
        sportradar_id = player.get('sportradar_id', '')
        if pd.isna(sportradar_id):
            sportradar_id = ''
        ...
        # Get position
        position = player.get(position_col, '')
        if pd.isna(position):
            position = ''
        ...
        for name in names_to_try:
            if name:
                player_lookup[name] = {
                    'sportradar_id': str(sportradar_id),
                    'player_name': str(player_name),
                    'position': str(position),
                    'headshot_url': str(headshot),
                    'recent_team': str(team)
                }
    
    print(f"Created lookup for {len(player_lookup)} player name variations")
    
    # Sample debug: show a few entries
    print("\nSample player lookup entries:")
    for i, (name, info) in enumerate(list(player_lookup.items())[:3]):
        print(f"  '{name}': sportradar_id='{info['sportradar_id']}', team='{info['recent_team']}'")
    ...
    for position in tqdm(positions, desc="Processing positions"):
        ...
        for _, pred in df_pred.iterrows():
            ...
            if player_info_found:
                ...
                clean_player = {
                    'sportradar_id': player_info_found['sportradar_id'],
                    'player_name': player_info_found['player_name'],
                    'position': player_info_found['position'],
                ...
                # Get sportradar_id from prediction if available
                pred_sr_id = pred.get('sportradar_id', '')
                if pd.isna(pred_sr_id):
                    pred_sr_id = ''
                
                # Get team from prediction
                pred_team = pred.get('team', '')
                if pd.isna(pred_team):
                    pred_team = ''
                
                clean_player = {
                    'sportradar_id': str(pred_sr_id),
        ...
        if not df_clean.empty:
            ...
            for _, player in df_clean.head(5).iterrows():
                team_str = player['team'] if player['team'] else 'N/A'
                sr_id_str = player['sportradar_id'][:10] + '...' if player['sportradar_id'] else 'N/A'
                print(f"    {player['player_name']} ({team_str}, SR:{sr_id_str}): ${player['price']:,} ({player['projected_points']} pts)")
...
def create_combined_player_pool(season, week, output_directory="player_pools"):
    ...
    if all_players:
        ...
        df_combined = df_combined.sort_values(['position', 'price'], ascending=[True, False])
        ...
        print(f"Players by position: {df_combined['position'].value_counts().to_dict()}")
        
        # Check for missing data
        missing_sr_ids = df_combined['sportradar_id'].isna().sum() + (df_combined['sportradar_id'] == '').sum()
        missing_teams = df_combined['team'].isna().sum() + (df_combined['team'] == '').sum()
        
        print(f"\nData completeness:")
        print(f"  Missing sportradar_id: {missing_sr_ids}/{len(df_combined)}")
        print(f"  Missing team: {missing_teams}/{len(df_combined)}")
...
def main():
    ...
    parser.add_argument('--main-dataset', type=str, default='data/nfl_dataset.csv',
                       help='Path to main dataset (default: data/nfl_dataset.csv)')
